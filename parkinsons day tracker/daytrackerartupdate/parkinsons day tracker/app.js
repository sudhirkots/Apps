...
/* One cartoon per state: the same woman and the same chair in all three,
   attempting the same thing — getting out of it. Only the ease changes.
   Each PNG has its editable vector source beside it as a .svg. */
const ART = {
  red: 'assets/red-freezing.png',
  yellow: 'assets/yellow-standing.png',
  green: 'assets/green-dyskinesia.png'
};
...
function colourButtons(compact) {
  ...
  const btn = (color, cssName, name, what) =>
    '<button type="button" class="' + cls + ' ' + cssName + '" data-act="pick" data-v="' + color + '">' +
      '<span class="thumb"><img src="' + ART[color] + '" alt=""></span>' +
      '<span class="lines"><span class="name">' + name + '</span>' +
      '<span class="what">' + what + '</span></span></button>';

  return '<div class="colours">' +
    btn('red', 'cb-red', nameRed, whatRed) +
    btn('yellow', 'cb-yellow', nameYel, whatYel) +
    btn('green', 'cb-green', nameGrn, whatGrn) +
    '</div>';
}
...
function viewStepRed() {
  return stepScreen('red',
    '<img src="' + ART.red + '" alt="Struggling to get up from a chair">',
    'Red — you are off',
...
function viewStepYellow() {
  return stepScreen('yellow',
    '<img src="' + ART.yellow + '" alt="Getting up from a chair easily">',
    'Yellow — you are on',
...
function viewStepGreen() {
  return stepScreen('green',
    '<img src="' + ART.green + '" alt="Seated, thrown about by movements she cannot control">',
    'Green — too much',
